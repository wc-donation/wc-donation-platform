...
$min_donation_amount = floatval(get_option('wcdp_min_amount') ?? 3) ?? 3;
$max_donation_amount = floatval(get_option('wcdp_max_amount') ?? 5000) ?? 5000;
...
$wcdp_price_field = sprintf( get_woocommerce_price_format(), '<span class="woocommerce-Price-currencySymbol">' . get_woocommerce_currency_symbol() . '</span>', '<input type="number" class="wcdf-input-field validate-required %s" id="wcdp-donation-amount" name="wcdp-donation-amount" step="%s" min="%s" max="%s" value="%s" required>' );
