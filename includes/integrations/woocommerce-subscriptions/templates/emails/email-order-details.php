...
if ('cancelled_subscription' != $email->id) {
    ...
    if ('order' == $order_type) {
        ...
        printf(esc_html_x('%1$sDonation #%3$s%2$s (%4$s)', 'Used in email notification', 'wc-donation-platform'), '<a href="' . esc_url($link_element_url) . '">', '</a>', esc_html($order->get_order_number()), sprintf('<time datetime="%s">%s</time>', esc_attr(wcs_get_objects_property($order, 'date_created')->format('c')), esc_html(wcs_format_datetime(wcs_get_objects_property($order, 'date_created')))));
    } else {
        // translators: $1-$3: opening and closing <a> tags $2: subscription's order number
        printf(esc_html_x('Recurring Donation %1$s#%2$s%3$s', 'Used in email notification', 'wc-donation-platform'), '<a href="' . esc_url($link_element_url) . '">', esc_html($order->get_order_number()), '</a>');
    }
    echo '</h2>';
}
?>
<div style="margin-bottom: 40px;">
    <table class="td" cellspacing="0" cellpadding="6"
            ...
            foreach ($totals as $total) {
                ...
                <tr>
                    <th class="td" scope="row" colspan="2"
                        style="text-align:<?php echo esc_attr($text_align); ?>; <?php if (1 == $i) {
                            echo 'border-top-width: 4px;';
                        } ?>"><?php echo esc_html($total['label']); ?></th>
                    <td class="td" style="text-align:<?php echo esc_attr($text_align); ?>; <?php if (1 == $i) {
                        echo 'border-top-width: 4px;';
        ...
        if ($order->get_customer_note()) {
            ...
            <tr>
                <th class="td" scope="row" colspan="2"
                    style="text-align:<?php echo esc_attr($text_align); ?>;"><?php esc_html_e('Note:', 'woocommerce-subscriptions'); ?></th>
                <td class="td"
