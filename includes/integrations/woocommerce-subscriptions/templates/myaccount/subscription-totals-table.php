...
<table class="shop_table order_details">
    ...
    <tr>
        ...
        <?php endif; ?>
        <th class="product-name"><?php echo esc_html_x('Donation', 'table headings in notification email', 'wc-donation-platform'); ?></th>
                ...
                <?php if ($allow_item_removal) : ?>
                    <td class="remove_item">
                        <?php if (wcs_can_item_be_removed($item, $subscription)) : ?>
                            <?php $confirm_notice = apply_filters('woocommerce_subscriptions_order_item_remove_confirmation_text', __('Are you sure you want remove this item from your recurring donation?', 'wc-donation-platform'), $item, $_product, $subscription); ?>
                            <a href="<?php echo esc_url(WCS_Remove_Item::get_remove_url($subscription->get_id(), $item_id)); ?>"
                               class="remove" onclick="return confirm('<?php printf(esc_html($confirm_notice)); ?>');">&times;</a>
                        <?php endif; ?>
                    </td>
                <?php endif; ?>
