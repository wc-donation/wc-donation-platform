        ...
        foreach ( $customer_orders->orders as $customer_order ) {
            ...
            <tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
                ...
                <?php elseif ( $is_order_number ) : ?>
                    ...
                    <a href="<?php echo esc_url( $order->get_view_order_url() ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'View donation number %s', 'wc-donation-platform' ), $order->get_order_number() ) ); ?>">
                ...
                <?php elseif ( 'order-total' === $column_id ) : ?>
                    ...
                    echo wp_kses_post( sprintf( _n( '%1$s', '%1$s for %2$s items', $item_count, 'woocommerce' ), $order->get_formatted_order_total(), $item_count ) );
                    ?>
...
<?php else : ?>
    ...
    <?php wc_print_notice( esc_html__( 'No donation has been made yet.', 'wc-donation-platform' ) . ' <a class="woocommerce-Button wc-forward button' . esc_attr( $wp_button_class ) . '" href="' . esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ) . '">' . esc_html__( 'Browse', 'wc-donation-platform' ) . '</a>', 'notice' ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingHookComment ?>
